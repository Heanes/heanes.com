...
class Captcha {
	...
	public function display() {
		if(function_exists('imagecreate') && function_exists('imagecolorset') && function_exists('imagecopyresized') &&
			...
			function_exists('imageline') && function_exists('imagecreatefromstring') && (function_exists('imagegif')
	 ...
	 * @doc 绘制验证码图片
	 * @author Heanes
	 * @time 2015-06-08 16:47:35
	 */
	function image() {
		$bg_content = $this->background();
		$this->im = imagecreatefromstring($bg_content);
	 ...
	 * @return string{tags}
	 * @author Heanes
	 * @time 2015-06-08 16:17:56
	 */
	function background() {
		$this->im = imagecreatetruecolor($this->width, $this->height);
		$captcha_length=strlen($this->code);
		$backgrounds = $c = array();
		if(!$this->background || !$backgrounds) {
			$step=array();
			for($i = 0;$i < $captcha_length-1;$i++) {
	 ...
	 * @doc 画干扰线
	 * @author Heanes
	 * @time 2015-06-08 16:18:19
	 */
	function adulterate() {
		$line_nums = $this->height / 20;
		for($i = 0; $i <= $line_nums;$i++) {
			$color = $this->color ? imagecolorallocate($this->im, mt_rand(0, 255), mt_rand(0, 255), mt_rand(0, 255)) : imagecolorallocate($this->im, $this->font_color[0], $this->font_color[1], $this->font_color[2]);
	 ...
	 * @doc 字体
	 * @author Heanes
	 * @time 2015-06-08 16:08:52
	 */
	function giffont() {
		$captcha = $this->code;
		$captcha_length=strlen($this->code);
		//echo $captcha_length;exit;
		$captcha_dir = array();
		$captcha_root='';
		if(function_exists('imagecreatefromgif')) {
			$captcha_root = $this->data_path.'gif/';
			$dirs = opendir($captcha_root);
			while($dir = readdir($dirs)) {
				if($dir != '.' && $dir != '..' && file_exists($captcha_root.$dir.'/9.gif')) {
					$captcha_dir[] = $dir;
				}
			}
		}
		...
		$font=array();
		for($i = 0; $i <= $captcha_length-1; $i++) {
			$this->imcodefile = $captcha_dir ? $captcha_root.$captcha_dir[array_rand($captcha_dir)].'/'.strtolower($captcha[$i]).'.gif' : '';
		...
		$x = mt_rand(1, $this->width - $width_total);
		for($i = 0; $i <= $captcha_length-1; $i++) {
			$this->color && $this->font_color = array(mt_rand(0, 255), mt_rand(0, 255), mt_rand(0, 255));
			if($font[$i]['file']) {
				...
				imagecolorset($this->imcode, 0 , $this->font_color[0], $this->font_color[1], $this->font_color[2]);
				...
				if($this->shadow) {
					$text_shadow_color = imagecolorallocate($this->im, 0, 0, 0);
					imagechar($this->im, 5, $x + 1, $y + 1, $captcha[$i], $text_shadow_color);
				}
				$text_color = imagecolorallocate($this->im, $this->font_color[0], $this->font_color[1], $this->font_color[2]);
