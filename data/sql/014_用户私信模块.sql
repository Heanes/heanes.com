...
create table `pre_user_message` (
    ...
    `send_time`         int(10) unsigned comment '发送时间',
    `is_read`           tinyint comment '是否已读',
    `read_time`         int(10) unsigned comment '阅读时间',
    `delete_time`       int(10) unsigned comment '删除时间',
    `is_recycle`        tinyint unsigned comment '是否放入回收站',
    `recycle_time`      int(10) unsigned comment '回收时间',
    `is_emergency`      tinyint comment '是否紧急',
    `is_timing_auto`    tinyint comment '是否定时发送',
    `auto_send_time`    int(10) unsigned comment '定时发送时间',
    `is_time_limit`     tinyint comment '是否具有有效期',
    `limit_time_end`    int(10) unsigned comment '失效时间',
    `is_top`            tinyint comment '是否置顶',
    `top_time_start`    int(10) unsigned comment '置顶开始时间',
    `top_time_end`      int(10) unsigned comment '置顶结束时间',
