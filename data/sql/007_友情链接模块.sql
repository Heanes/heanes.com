...
create table `pre_friend_link_category` (
    ...
    `insert_time`   int(10) unsigned comment '分类添加时间',
    `update_time`   int(10) unsigned comment '分类最后更新时间',
...
create table `pre_friend_link` (
    ...
    `description`   text comment '备注介绍',
    `order_number`  int unsigned comment '排序',
    `insert_time`   int(10) unsigned comment '链接添加时间',
    `update_time`   int(10) unsigned comment '链接更新时间',
    `is_enable`     tinyint unsigned default 1 comment '是否启用',
    `is_deleted`    tinyint unsigned default 0 comment '是否删除',
    `create_user`   int unsigned comment '创建人',
    `update_user`   int unsigned comment '更新人',
