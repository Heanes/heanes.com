...
create table `pre_borrow` (
    ...
    `get_money_limit_time`  int(10) unsigned comment '贷款成功截止期限',
    `get_money_time`        int(10) unsigned comment '放款时间',
    `repay_money_time`      int(10) unsigned comment '还款时间',
    `has_colleague`         tinyint comment '是否有同行',
    `apply_time`            int(10) unsigned comment '贷款申请时间（发布时间）',
    `apply_status`          tinyint comment '贷款申请状态，0-审核中，1-已通过，2-已拒绝',
    `apply_update_time`     int(10) unsigned comment '贷款申请最后更新时间',
    `progress_status`       smallint comment '进行状态，1-下户，2-评级，3-做卷，3-资料审核，4-批贷函，5-贷后管理（开始放款）',
    `progress_update_time`  int(10) unsigned comment '最后更新时间',
    `order_number`          int comment '排序',
    `is_enable`             tinyint default 1 comment '是否有效',
    `is_deleted`            tinyint default 0 comment '是否删除',
    `insert_time`           int(10) unsigned comment '插入时间',
...
create table `pre_borrow_progress` (
    ...
    `is_enable`     tinyint default 1 comment '是否有效',
    `is_deleted`    tinyint default 0 comment '是否删除',
    `insert_time`   int(10) unsigned comment '进度插入时间',
    primary key (`id`)
)
engine = innodb
auto_increment = 1
...
create table `pre_borrow_category` (
    ...
    `access_password`   varchar(64) comment '分类访问密码',
...
create table `pre_borrow_fields_data` (
    ...
    `is_enable`     tinyint unsigned default 1 comment '是否启用',
    `is_deleted`    tinyint unsigned default 0 comment '是否删除',
    `insert_time`   int(10) unsigned comment '属性添加时间',
    `update_time`   int(10) unsigned comment '属性更新时间',
    `create_user`   int unsigned comment '创建人',
    `update_user`   int unsigned comment '更新人',
    primary key (`id`)
)
