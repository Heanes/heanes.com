...
/**
 * @doc 用户模型
 * @filesource JiekuanModel.class.php
 * @copyright heanes.com
 * @author Carr
 * @time 2015-07-09 17:54:56
 */
defined('InHeanes') or exit('Access Invalid!');
class JiekuanModel extends BaseModel {
	
	function __construct($table_name = 'jiekuan') {
	...
	public function addUser($new_user){
		//插入数据
		if($new_user=DB::insert('jiekuan',$new_user)){
			return $new_user;
		}else{
			return $new_user;
		}
	...
	public function updateUser($update_user_data,$where){
		if(DB::update('jiekuan',$update_user_data,$where)){
			return true;
		}else{
			return false;
		}
	...
	public function deleteUser($where){
		return DB::delete('jiekuan',$where);
	}
