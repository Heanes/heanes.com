...
<div class="data-list-href">
	<div class="handle-field">
		<a href="<?php echo BASE_URL;?>index.php?act=GoodsType&op=add" class="btn btn-primary btn-large">添加商品类型</a>
	</div>
</div>
<!-- 数据列表 S -->
<div class="data-list-table">
	<table class="table table-striped table-bordered table-condensed table-data-list">
		...
		<tbody>
		<?php if(empty($output['goodsType_list'])){ ?>
		<tr><td>暂无内容</td></tr>
		<?php } ?>
		<?php foreach ($output['goodsType_list'] as $key=>$goodsType) { ?>
		<tr>
			<td style="text-align:center;"><input name="check" type="checkbox"></td>
			<td style="text-align:center;"><?php echo $goodsType['id'];?></td>
			<td style="text-align:center;"><a href="<?php echo BASE_URL;?>index.php?act=GoodsType&op=edit&id=<?php echo $goodsType['id'];?>"><?php echo $goodsType['name'];?></a></td>
			<td style="text-align:center;"><?php echo to_date($goodsType['create_time']);?></td>
			<td style="text-align:center;"><?php echo to_date($goodsType['update_time']);?></td>
			<td style="text-align:center;"><?php echo $goodsType['is_enable']==1?'显示':'不显示';?></td>
			<td style="text-align:center;"><?php echo $goodsType['is_delete']==1?'是':'否';?></td>
			<td style="text-align:center;">
				<a href="<?php echo BASE_URL;?>index.php?act=GoodsType&op=edit&id=<?php echo $goodsType['id'];?>" class="btn btn-mini">编辑</a>
				<a href="javascript:del('goods_type', <?php echo $goodsType['id'];?>);" class="btn btn-mini btn-danger">删除</a>
			</td>
		</tr>
		<?php } ?>
		</tbody>
