...
<div class="data-list-href">
	<div class="handle-field">
		<a href="<?php echo BASE_URL;?>index.php?act=ProAttribute&op=add" class="btn btn-primary btn-large">添加产品属性名称</a>
	</div>
</div>
<!-- 数据列表 S -->
<div class="data-list-table">
	<table class="table table-striped table-bordered table-condensed table-data-list">
		...
		<tbody>
		<?php if(empty($output['productFields_list'])){ ?>
		<tr><td>暂无内容</td></tr>
		<?php } ?>
		<?php foreach ($output['productFields_list'] as $key=>$productFields) { ?>
		<tr>
			<td style="text-align:center;"><input name="check" type="checkbox"></td>
			<td style="text-align:center;"><?php echo $productFields['id'];?></td>
			<td style="text-align:center;"><a href="<?php echo BASE_URL;?>index.php?act=ProAttribute&op=edit&id=<?php echo $productFields['id'];?>"><?php echo $productFields['name'];?></a></td>
			<td style="text-align:center;"><?php echo $productFields['input_type'];?></td>
			<td style="text-align:center;"><?php echo $productFields['type_name'];?></td>
			<td style="text-align:center;"><?php echo $productFields['as_filter']==1?'是':'否';?></td>
			<td style="text-align:center;"><?php echo $productFields['is_show']==1?'是':'否';?></td>
			<td style="text-align:center;"><?php echo $productFields['is_enable']==1?'是':'否';?></td>
			<td style="text-align:center;"><?php echo $productFields['is_delete']==1?'是':'否';?></td>
			<td style="text-align:center;">
				<a href="<?php echo BASE_URL;?>index.php?act=ProAttribute&op=edit&id=<?php echo $productFields['id'];?>" class="btn btn-mini">编辑</a>
				<a href="javascript:del('product_fields', <?php echo $productFields['id'];?>);" class="btn btn-mini btn-danger">删除</a>
			</td>
		</tr>
		<?php } ?>
		</tbody>
