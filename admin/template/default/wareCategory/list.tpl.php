...
<div class="data-list-href">
	<div class="handle-field">
		<a href="<?php echo BASE_URL;?>index.php?act=WareCategory&op=add" class="btn btn-primary btn-large">添加物品分类</a>
	</div>
</div>
<!-- 数据列表 S -->
<div class="data-list-table">
	<table class="table table-striped table-bordered table-condensed table-data-list">
		...
		<tbody>
		<?php if(empty($output['wareCategory_list'])){ ?>
		<tr><td>暂无内容</td></tr>
		<?php } ?>
		<?php foreach ($output['wareCategory_list'] as $key=>$wareCategory) { ?>
		<tr>
			<td style="text-align:center;"><input name="check" type="checkbox"></td>
			<td style="text-align:center;"><?php echo $wareCategory['id'];?></td>
			<td style="text-align:center;"><a href="<?php echo BASE_URL;?>index.php?act=WareCategory&op=edit&id=<?php echo $wareCategory['id'];?>"><?php echo $wareCategory['name'];?></a></td>
			<td style="text-align:center;"><?php echo $wareCategory['a_href'];?></td>
			<td style="text-align:center;"><?php echo $wareCategory['img_src'];?></td>
			<td style="text-align:center;"><?php echo $wareCategory['user_role_name'];?></td>
			<td style="text-align:center;"><?php echo $wareCategory['user_rank'];?></td>
			<td style="text-align:center;"><?php echo to_date($wareCategory['create_time']);?></td>
			<td style="text-align:center;"><?php echo to_date($wareCategory['update_time']);?></td>
			<td style="text-align:center;"><?php echo $wareCategory['is_enable']==1?'显示':'不显示';?></td>
			<td style="text-align:center;"><?php echo $wareCategory['is_delete']==1?'是':'否';?></td>
			<td style="text-align:center;">
				<a href="<?php echo BASE_URL;?>index.php?act=WareCategory&op=edit&id=<?php echo $wareCategory['id'];?>" class="btn btn-mini">编辑</a>
				<a href="javascript:del('ware_category', <?php echo $wareCategory['id'];?>);" class="btn btn-mini btn-danger">删除</a>
			</td>
		</tr>
		<?php } ?>
		</tbody>
