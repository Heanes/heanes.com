...
<div class="data-list-href">
	<div class="handle-field">
		<a href="<?php echo BASE_URL;?>index.php?act=borrowProgress&op=add" class="btn btn-primary btn-large">添加贷款进度</a>
	</div>
</div>
<!-- 数据列表 S -->
<div class="data-list-table">
	<table class="table table-striped table-bordered table-condensed table-data-list">
		...
		<tr>
			...
			<th>
				<a href="javascript:sortBy('jk_id', 'DESC');" title="点击对列表排序">业务主ID<em class="triangle-down"></em></a>
			</th>
			<th>
				<a href="javascript:sortBy('actor_user_id', 'DESC');" title="点击对列表排序">操作人ID<em class="triangle-up"></em></a>
			</th>
			<th>
		...
		<?php if(empty($output['borrowProgressList'])){ ?>
		<tr><td>暂无内容</td></tr>
		<?php } ?>
		<?php foreach ($output['borrowProgressList'] as $key=>$borrowProgress) { ?>
		<tr>
			<td style="text-align:center;"><input name="check" type="checkbox"></td>
			<td style="text-align:center;"><?php echo $borrowProgress['id'];?></td>
			...
			<td style="text-align:center;">
				<a href="<?php echo BASE_URL;?>index.php?act=borrowProgress&op=edit&id=<?php echo $borrowProgress['id'];?>" class="btn btn-mini">编辑</a>
				<a href="javascript:del('borrow_progress', <?php echo $borrowProgress['id'];?>);" class="btn btn-mini btn-danger">删除</a>
			</td>
		</tr>
		<?php } ?>
		</tbody>
