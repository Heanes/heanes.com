...
<div class="data-list-href">
	<div class="handle-field">
		<a href="<?php echo BASE_URL;?>index.php?act=UserAttributeData&op=add" class="btn btn-primary btn-large">添加用户额外属性字段数据映射</a>
	</div>
</div>
<!-- 数据列表 S -->
<div class="data-list-table">
	<table class="table table-striped table-bordered table-condensed table-data-list">
		<thead>
		<tr>
			<td colspan="15">用户额外属性字段数据映射列表</td>
		</tr>
		<tr>
			...
			<th>
				<a href="javascript:listTable.sort('fields_id', 'DESC');" title="点击对列表排序">注册项ID<em class="triangle-down"></em></a>
			</th>
			<th>
		...
		<tbody>
		<?php if(empty($output['userFieldsData_list'])){ ?>
		<tr><td>暂无内容</td></tr>
		<?php } ?>
		<?php foreach ($output['userFieldsData_list'] as $key=>$userFieldsData) { ?>
		<tr>
			<td style="text-align:center;"><input name="check" type="checkbox"></td>
			<td style="text-align:center;"><?php echo $userFieldsData['id'];?></td>
			...
			<td style="text-align:center;">
				...
				<a href="javascript:del('user_fields_data', <?php echo $userFieldsData['id'];?>);" class="btn btn-mini btn-danger">删除</a>
			</td>
		</tr>
		<?php } ?>
		</tbody>
