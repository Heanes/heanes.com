...
<div class="data-list-href">
	<div class="handle-field">
		<a href="<?php echo BASE_URL;?>index.php?act=slideWap&op=add" class="btn btn-primary btn-large">添加幻灯WAP端</a>
	</div>
</div>
<!-- 数据列表 S -->
<div class="data-list-table">
	<table class="table table-striped table-bordered table-condensed table-data-list">
		...
		<tr>
			...
			<th>
				<a href="javascript:sortBy('title', 'DESC');" title="点击对列表排序">显示标题<em class="triangle-down"></em></a>
			</th>
			<th>
		...
		<tbody>
		<?php if(empty($output['slideWap_list'])){ ?>
		<tr><td>暂无内容</td></tr>
		<?php } ?>
		<?php foreach ($output['slideWap_list'] as $key=>$slideWap) { ?>
		<tr>
			<td style="text-align:center;"><input name="check" type="checkbox"></td>
			<td style="text-align:center;"><?php echo $slideWap['id'];?></td>
			<td style="text-align:center;"><a href="<?php echo BASE_URL;?>index.php?act=slideWap&op=edit&id=<?php echo $slideWap['id'];?>"><?php echo $slideWap['name'];?></a></td>
			<td style="text-align:center;"><?php echo $slideWap['img_src'];?></td>
			<td style="text-align:center;"><?php echo $slideWap['a_href'];?></td>
			<td style="text-align:center;"><?php echo $slideWap['a_target']==1?'是':'否';?></td>
			<td style="text-align:center;"><?php echo $slideWap['title'];?></td>
			<td style="text-align:center;"><?php echo to_date($slideWap['insert_time']);?></td>
			<td style="text-align:center;"><?php echo to_date($slideWap['update_time']);?></td>
			<td style="text-align:center;"><?php echo $slideWap['is_enable']==1?'显示':'不显示';?></td>
			<td style="text-align:center;"><?php echo $slideWap['is_delete']==1?'是':'否';?></td>
			<td style="text-align:center;">
				<a href="<?php echo BASE_URL;?>index.php?act=slideWap&op=edit&id=<?php echo $slideWap['id'];?>" class="btn btn-mini">编辑</a>
				<a href="javascript:del('slide_wap', <?php echo $slideWap['id'];?>);" class="btn btn-mini btn-danger">删除</a>
			</td>
		</tr>
		<?php } ?>
		</tbody>
