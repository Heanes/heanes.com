...
<form action="<?php echo BASE_URL.'index.php?act=department&op=update';?>" method="post" class="input-condensed" enctype="multipart/form-data">
	<table class="table table-striped table-condensed table-data-edit">
		...
		<tbody>
			...
			<tr>
				<th>创建时间</th>
				<td><input type="text" name="department_insert_time" id="department_insert_time1" value="<?php echo to_date($output['department']['insert_time']);?>" placeholder="选择起始时间" onclick="$.calendar({format:'yyyy-MM-dd HH:mm:ss'});" class="date_time_picker"/></td>
				<th>修改时间</th>
				<td><input type="text" name="department_update_time" id="department_insert_time2" value="<?php echo to_date(getGMTime());?>" placeholder="选择起始时间" onclick="$.calendar({format:'yyyy-MM-dd HH:mm:ss'});" class="date_time_picker"/></td>
			</tr>
			<tr>
				<th>是否有效</th>
				<td>
