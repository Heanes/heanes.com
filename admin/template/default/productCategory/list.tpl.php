...
<div class="data-list-href">
	<div class="handle-field">
		<a href="<?php echo BASE_URL;?>index.php?act=ProductCategory&op=add" class="btn btn-primary btn-large">添加产品分类</a>
	</div>
</div>
<!-- 数据列表 S -->
<div class="data-list-table">
	<table class="table table-striped table-bordered table-condensed table-data-list">
		...
		<tbody>
		<?php if(empty($output['productCategory_list'])){ ?>
		<tr><td>暂无内容</td></tr>
		<?php } ?>
		<?php foreach ($output['productCategory_list'] as $key=>$productCategory) { ?>
		<tr>
			<td style="text-align:center;"><input name="check" type="checkbox"></td>
			<td style="text-align:center;"><?php echo $productCategory['id'];?></td>
			<td style="text-align:center;"><a href="<?php echo BASE_URL;?>index.php?act=ProductCategory&op=edit&id=<?php echo $productCategory['id'];?>"><?php echo $productCategory['name'];?></a></td>
			<td style="text-align:center;"><?php echo $productCategory['a_href'];?></td>
			<td style="text-align:center;"><?php echo $productCategory['img_src'];?></td>
			<td style="text-align:center;"><?php echo $productCategory['user_role_name'];?></td>
			<td style="text-align:center;"><?php echo $productCategory['user_rank'];?></td>
			<td style="text-align:center;"><?php echo to_date($productCategory['create_time']);?></td>
			<td style="text-align:center;"><?php echo to_date($productCategory['update_time']);?></td>
			<td style="text-align:center;"><?php echo $productCategory['is_enable']==1?'显示':'不显示';?></td>
			<td style="text-align:center;"><?php echo $productCategory['is_delete']==1?'是':'否';?></td>
			<td style="text-align:center;">
				<a href="<?php echo BASE_URL;?>index.php?act=ProductCategory&op=edit&id=<?php echo $productCategory['id'];?>" class="btn btn-mini">编辑</a>
				<a href="javascript:del('product_category', <?php echo $productCategory['id'];?>);" class="btn btn-mini btn-danger">删除</a>
			</td>
		</tr>
		<?php } ?>
		</tbody>
