...
<form action="<?php echo BASE_URL.'index.php?act=navigation&op=insert';?>" method="post" class="input-condensed" enctype="multipart/form-data">
	<table class="table table-striped table-condensed table-data-edit">
		...
		<tr>
			<th>内部ID</th>
			<td><input type="text" name="navigation_id" readonly value="<?php echo $output['lastID'];?>"></td>
			<th>排序</th>
			<td><input type="text" name="order" value=""></td>
		</tr>
		<tr>
			<th><span class="need">*</span>导航栏名称</th>
			<td><input type="text" name="navigation_name" value="" style="width:60%;"></td>
			<th>上级导航名称</th>
			<td>
				<select name="parent_id">
					<option value="0">顶级</option>
					<?php foreach ($output['navigationArr'] as $key => $navigationArr) {?>
					<option value ="<?php echo $navigationArr['id'];?>"><?php echo $navigationArr['name'];?></option>
					<?php }?>
				</select>
			</td>
		</tr>
		...
		<tr>
			<th>导航创建时间</th>
			<td><input type="text" name="navigation_insert_time" id="navigation_insert_time1" value="<?php echo to_date('now');?>" placeholder="选择创建时间" onclick="$.calendar({format:'yyyy-MM-dd HH:mm:ss'});" class="date_time_picker"/></td>
			<th>导航更新时间</th>
			<td><input type="text" name="navigation_update_time" id="navigation_update_time2" value="<?php echo to_date('now');?>" placeholder="选择更新时间" onclick="$.calendar({format:'yyyy-MM-dd HH:mm:ss'});" class="date_time_picker"/></td>
		</tr>
		<tr>
			<th>是否启用</th>
			<td>
