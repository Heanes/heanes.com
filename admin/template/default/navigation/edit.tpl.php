...
<form action="<?php echo BASE_URL.'index.php?act=navigation&op=update';?>" method="post" class="input-condensed" enctype="multipart/form-data">
	<table class="table table-striped table-condensed table-data-edit">
		...
		<tbody>
			...
			<tr>
				<th>导航创建时间</th>
				<td><input type="text" name="navigation_insert_time" id="navigation_insert_time1" value="<?php echo to_date($output['navigation']['insert_time']);?>" placeholder="选择创建时间" onclick="$.calendar({format:'yyyy-MM-dd HH:mm:ss'});" class="date_time_picker"/></td>
				<th>导航更新时间</th>
				<td><input type="text" name="navigation_update_time" id="navigation_insert_time2" value="<?php echo to_date(getGMTime());?>" placeholder="选择更新时间" onclick="$.calendar({format:'yyyy-MM-dd HH:mm:ss'});" class="date_time_picker"/></td>
			</tr>
			<tr>
				<th>导航链接图标激活样式地址</th>
