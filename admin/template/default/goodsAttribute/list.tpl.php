...
<div class="data-list-href">
	<div class="handle-field">
		<a href="<?php echo BASE_URL;?>index.php?act=GoodsAttribute&op=add" class="btn btn-primary btn-large">添加商品属性名称</a>
	</div>
</div>
<!-- 数据列表 S -->
<div class="data-list-table">
	<table class="table table-striped table-bordered table-condensed table-data-list">
		...
		<tbody>
		<?php if(empty($output['goodsFields_list'])){ ?>
		<tr><td>暂无内容</td></tr>
		<?php } ?>
		<?php foreach ($output['goodsFields_list'] as $key=>$goodsFields) { ?>
		<tr>
			<td style="text-align:center;"><input name="check" type="checkbox"></td>
			<td style="text-align:center;"><?php echo $goodsFields['id'];?></td>
			<td style="text-align:center;"><?php echo $goodsFields['type_name'];?></td>
			<td style="text-align:center;"><a href="<?php echo BASE_URL;?>index.php?act=GoodsAttribute&op=edit&id=<?php echo $goodsFields['id'];?>"><?php echo $goodsFields['name'];?></a></td>
			<td style="text-align:center;"><?php echo $goodsFields['input_type'];?></td>
			<td style="text-align:center;"><?php echo $goodsFields['as_filter']==1?'是':'否';?></td>
			<td style="text-align:center;"><?php echo $goodsFields['is_show']==1?'是':'否';?></td>
			<td style="text-align:center;"><?php echo $goodsFields['is_enable']==1?'是':'否';?></td>
			<td style="text-align:center;"><?php echo $goodsFields['is_delete']==1?'是':'否';?></td>
			<td style="text-align:center;">
				<a href="<?php echo BASE_URL;?>index.php?act=GoodsAttribute&op=edit&id=<?php echo $goodsFields['id'];?>" class="btn btn-mini">编辑</a>
				<a href="javascript:del('goods_fields', <?php echo $goodsFields['id'];?>);" class="btn btn-mini btn-danger">删除</a>
			</td>
		</tr>
		<?php } ?>
		</tbody>
