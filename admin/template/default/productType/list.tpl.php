...
<div class="data-list-href">
	<div class="handle-field">
		<a href="<?php echo BASE_URL;?>index.php?act=ProductType&op=add" class="btn btn-primary btn-large">添加产品类型</a>
	</div>
</div>
<!-- 数据列表 S -->
<div class="data-list-table">
	<table class="table table-striped table-bordered table-condensed table-data-list">
		...
		<tbody>
		<?php if(empty($output['productType_list'])){ ?>
		<tr><td>暂无内容</td></tr>
		<?php } ?>
		<?php foreach ($output['productType_list'] as $key=>$productType) { ?>
		<tr>
			<td style="text-align:center;"><input name="check" type="checkbox"></td>
			<td style="text-align:center;"><?php echo $productType['id'];?></td>
			<td style="text-align:center;"><a href="<?php echo BASE_URL;?>index.php?act=ProductType&op=edit&id=<?php echo $productType['id'];?>"><?php echo $productType['name'];?></a></td>
			<td style="text-align:center;"><?php echo to_date($productType['insert_time']);?></td>
			<td style="text-align:center;"><?php echo to_date($productType['update_time']);?></td>
			<td style="text-align:center;"><?php echo $productType['is_enable']==1?'显示':'不显示';?></td>
			<td style="text-align:center;"><?php echo $productType['is_delete']==1?'是':'否';?></td>
			<td style="text-align:center;">
				<a href="<?php echo BASE_URL;?>index.php?act=ProductType&op=edit&id=<?php echo $productType['id'];?>" class="btn btn-mini">编辑</a>
				<a href="javascript:del('product_type', <?php echo $productType['id'];?>);" class="btn btn-mini btn-danger">删除</a>
			</td>
		</tr>
		<?php } ?>
		</tbody>
