...
<div class="data-list-table">
	<table class="table table-striped table-bordered table-condensed table-data-list">
		...
		<tr>
			...
			<th>
				<a href="javascript:listTable.sort('user_rank_id', 'DESC');" title="点击对列表排序">用户积分ID<em class="triangle-down"></em></a>
			</th>
			<th>
				<a href="javascript:listTable.sort('chang_sign', 'DESC');" title="点击对列表排序">积分变更标识<em class="triangle-down"></em></a>
			</th>
			<th>
		...
		<tbody>
		<?php if(empty($output['userRankLog_list'])){ ?>
		<tr><td>暂无内容</td></tr>
		<?php } ?>
		<?php foreach ($output['userRankLog_list'] as $key=>$userRankLog) { ?>
		<tr>
			<td style="text-align:center;"><input name="check" type="checkbox"></td>
			<td style="text-align:center;"><?php echo $userRankLog['id'];?></td>
